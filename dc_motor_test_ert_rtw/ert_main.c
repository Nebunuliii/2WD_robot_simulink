...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(dc_motor_test_M) == (NULL));
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(dc_motor_test_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(dc_motor_test_M, "stopping the model");
}
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(dc_motor_test_M, 0);
  ...
  /* Initialize model */
  dc_motor_test_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(dc_motor_test_M->extModeInfo, &rtmGetTFinal
    (dc_motor_test_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(dc_motor_test_M, true);
    }
  }
